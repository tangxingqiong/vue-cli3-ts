// (function($) {
    // 当domReady的时候开始初始化
    var uploaderObj = function() {
        var $wrap = $('#uploader-box'),
        ...
        function updateStatus() {
            ...
            if (state === 'ready') {
                ...
                if (stats.uploadFailNum) {
                    text = '已成功上传' + stats.successNum + '文件，' + stats.uploadFailNum + '文件上传失败，<a class="retry" href="#">重新上传</a>失败或<a class="ignore" href="#">忽略</a>'
                }
        ...
        uploader.on('uploadSuccess',function(file,response){
            ...
            if(response['status']===0){
                var b=response['data'];
                var id = window.itemId;
                // var item_id = $('body').attr('item_id');
                // let aa = $('a[name=uploadImage][data='+id+']').parents('td')
                var item = $('a[name=uploadImage][data='+id+']').nextAll(".ul_pic_list");
                var c = item.parents("tr");
                var sFData_obj = c.parents('div.sData').prev('div.sFData');
                if(item.hasClass('ul_pic_list')){
                    item.append('<li class=' + 'del_img_pg' + '><img class="picture" url="'+b.url+'" src="' + window.apiURL + '/' + b.url + '"><i class=' + 'del_img_pg' + ' style="display: inline;"></i></li>');
                }
                sFData_obj.find('tr').eq(c.index()).height(c.height());
            }else{
