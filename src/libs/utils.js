 ...
 * 项目操作权限设置
 *  */
export function hasAuth(val, condition){
    let authList = store.state.user.projectAuthList
    // return authList.indexOf(val) > -1
    return authList.find(item => {
        let cdition = null
        if(item.condition){
            cdition = JSON.parse(item.condition)
        }
        let isName = (item.name == val && !condition)
        let isType = (item.name == val && condition && cdition && condition.conditionNum == 1 && cdition.company_type == condition.company_type)
        let TypeAndLevel = (item.name == val && condition && cdition && condition.conditionNum == 2 && cdition.company_type == condition.company_type && cdition.company_level == condition.company_level)
        if(isName || isType || TypeAndLevel){
            return true
        }
    })
}
